...
class Varastokerros {
	...
	async haeKuva(kuvatiedostonNimi) {
		if (this.#kuvakansioPolku && this.#kuvakansioPolku.length > 0) {
			try {
				const tiedostotunniste = path
					.extname(kuvatiedostonNimi)
					.toLocaleLowerCase();
	...
	async haeKuvalista() {
		...
		return kuvat.filter((kuva) =>
			tuetutTyypit.includes(path.extname(kuva).toLocaleLowerCase())
		);
  } // haeKuvalista loppu
  
  // Rest osuus
  async haeKaikkiVarastosta() {
    return (this.#lukija.lueVarasto(this.#varastopolku));
  }

  async haeYksiVarastosta(arvo) {
    return (await this.#lukija.lueVarasto(this.#varastopolku)).find(olio => olio[this.perusavain] == arvo) || null;
  }

  async lisaaVarastoon(uusiOlio) {
    const varasto = await this.#lukija.lueVarasto(this.#varastopolku);
    varasto.push(uusiOlio);
    return await this.#lukija.kirjoitaVarasto(this.#varastopolku, varasto);
  }

  async paivitaVarasto(olio) {
    const varasto = await this.#lukija.lueVarasto(this.#varastopolku);
    const vanhaOlio = varasto.find(vanha => vanha[this.perusavain] == olio[this.perusavain]);
    if (vanhaOlio) {
  ...
  async poistaVarastosta(arvo) {
    const varasto = await this.#lukija.lueVarasto(this.#varastopolku);
    const i = varasto.findIndex(alkio => alkio[this.perusavain] == arvo);
    if (i >= 0) return false;
    varasto.splice(i, 1);
    return await this.#lukija.kirjoitaVarasto(this.#varastopolku, varasto);
  }
